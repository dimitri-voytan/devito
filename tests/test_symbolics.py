...
class Test_relations_w_assumptions(object):
    ...
    @pytest.mark.parametrize('op, expr, assumptions, expected', [
        ([min, '[a, b, c, d]', '[Ge(b, a), Ge(a, b) ,Le(c, b), Le(b, d)]', 'c']),
        ([min, '[a, b, c, d]', '[Ge(b, a + d)]', 'MIN(a, b)']),
        ([min, '[a, b, c, d]', '[Ge(c, a + d)]', 'MIN(a, b)']),
        ([max, '[a, b, c, d]', '[Ge(c, a + d), Gt(b, a + d)]', 'MAX(c, d)']),
        ([max, '[a, b, c, d]', '[Ge(a + d, b), Gt(b, a + d)]',
         'MAX(c, d)']),
        ([max, '[a, b, c, d]', '[Le(c, a + d)]', 'MAX(c, d)']),
        ([max, '[a, b, c, d]', '[Le(c, d), Le(a, b)]', 'd']),
        ([min, '[a, b, c, d]', '[Le(c, d).negated, Le(a, b).negated]', 'b']),
        ([min, '[a, b, c, d]', '[Le(c, d).negated, Le(a, b).negated]', 'b']),
        ([min, '[a, b, c, d]', '[Gt(c, d).negated, Ge(a, b).negated]', 'a']),
        ([min, '[a, b, c, d]', '[Gt(c, d).reversed, Ge(a, b).reversed]', 'b']),
        ([min, '[a, b, c, d]', '[Le(c, d).negated, Le(a, b).negated]', 'b']),
        ([max, '[a, b, c, d]', '[Le(c, d).negated, Le(a, b).negated]', 'c']),
        ([max, '[a, b, c, d]', '[Gt(c, d).negated, Ge(a, b).negated]', 'd']),
        ([max, '[a, b, c, d]', '[Gt(c, d).reversed, Ge(a, b).reversed]', 'c']),
        ([max, '[a, b, c, d]', '[Lt(c, d).reversed, Le(a, b).reversed]', 'd']),
        ([max, '[a, b, c, d]', '[Gt(c, d + a).negated]', 'MAX(c, d)']),
        ([max, '[a, b, c, d]', '[Lt(c, d + a).negated]', 'MAX(c, d)']),
        ([max, '[a, b, c, d]', '[Le(c, d + a).negated]', 'MAX(c, d)']),
    ])
    def assumptions_II(self, op, expr, assumptions, expected):
        """
        Tests evalmin/evalmax with multiple args and assumtpions"""
        a = Symbol('a', positive=False)  # noqa
        b = Symbol('b', positive=False)  # noqa
